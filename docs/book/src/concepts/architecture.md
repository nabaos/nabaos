...
> - How the 5-tier pipeline, constitution, and caching work together
...
The orchestrator is the central event loop. Implemented as a Rust async runtime on top of tokio, it receives messages from all channels, routes them through security checks, dispatches them into the 5-tier pipeline, and returns responses. It also manages the task queue, cost tracking, and agent lifecycle.
...
Security runs **before** any LLM call, making it a hard gate rather than a suggestion. The layer comprises five subsystems that execute in sequence:
...
| **BERT Classifier** | 66M-parameter ONNX model that classifies query safety (safe / injection / out-of-scope) | 5-10ms |
| **Constitution Enforcer** | Matches the classified W5H2 intent against YAML constitution rules; returns allow, block, confirm, or warn | <1ms |
...
### Cache (Fingerprint + Intent Cache)

The cache system has two layers that work together:
...
- **Telegram** -- Primary channel. Supports inline keyboards for confirmation flows, Mini App for dashboards.
- **Discord** -- Slash commands and bot interactions.
- **Web** -- WebSocket-based interface for browser clients.

All channels share the same orchestrator, security layer, and pipeline. A query from Telegram is processed identically to one from Discord.
...
2. SECURITY GATE
   ...
   BERT Classifier: safe (confidence 0.98)           PASS
...
10. RESPONSE
    ...
    Log: latency=250ms, cost=$0.00, tier=2
```
...
2. **Similarity thresholds adapt** -- cache entries with high success rates relax their match threshold from 0.95 to 0.92, catching more variations
